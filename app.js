...
const chart = window.echarts ? echarts.init(document.getElementById('chartContainer')) : null;
  ...
  function groupOverviewSt(records) {
    ...
    const ensureGroup = (app,batch,country) => {
      // 兼容旧版数据：缺少国家字段时不要拆成空国家和正式国家两个 App。
      const normalizedCountry = country || '';
      const key = ['st',app,batch].join('|');
      if (!map.has(key)) map.set(key,{ source:'st', app, batch, country: normalizedCountry, records:[], statuses:[] });
      return map.get(key);
    };
    records.forEach(item => {
  ...
  function fillSelect(id, values, current, allLabel) {
    const element = document.getElementById(id);
    if (!element) return;
    const normalized = values.map(value => typeof value === 'object' ? value : ({value, label:value}));
    const options = [`<option value="">${escapeHtml(allLabel)}</option>`].concat(normalized.map(item => `<option value="${encode(item.value)}" ${item.value===current?'selected':''}>${escapeHtml(item.label)}</option>`));
